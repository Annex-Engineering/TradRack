...
class TradRack:
    ...
    cmd_TR_REMOVE_TOOL_NAME_help = (
        "Clear the name assigned to the specified tool"
    )
    ...
    cmd_TR_CHECK_FOR_NAME_help = (
        "Ensure the specified filament name is assigned to a tool"
    )
    ...
    def cmd_TR_CHECK_FOR_NAME(self, gcmd):
        ...
        gcmd.respond_info(
            "There is no tool with the name {name} assigned to it. Use"
            " TR_ASSIGN_TOOL_NAME TOOL=&lt;tool index&gt; NAME={name} to assign"
            " by tool or TR_ASSIGN_LANE LANE=&lt;lane index&gt; NAME={name} to"
            " assign by lane, then use TR_RESUME to continue.".format(name=name)
        )
        ...
        resume_kwargs = {
            ...
            "fail_msg": (
                "Cannot resume. Please use TR_ASSIGN_TOOL_NAME or"
                " TR_ASSIGN_LANE to assign the name %s to a tool or lane, then"
                " use TR_RESUME." % name
            ),
        }
        self._set_up_resume("check condition", resume_kwargs)
    ...
    def _assign_lane(self, lane, tool=None, name=None, id=None):
        ...
        if tool is not None and name is not None:
            ...
            if curr_tool is not None and tool != curr_tool:
                raise self.gcode.error(
                    'Name "{name}" is already assigned to tool {curr_tool}. To'
                    " assign it to tool {tool}, use TR_REMOVE_TOOL_NAME"
                    " TOOL={curr_tool} before trying again.".format(
                        name=name, curr_tool=curr_tool, tool=tool
                    )
                )
        ...
        if name is not None:
            ...
            # assign name to tool
                ...
                self.printer.send_event(
                    "trad_rack:assigned_name", curr_tool, name
                )
    ...
    def _select_lane(self, gcmd, lane=None, tool=None, name=None):
        ...
        gcmd.respond_info(
            "Tool {tool} has no lanes assigned to it. Use TR_ASSIGN_LANE"
            " LANE=&lt;lane index&gt; TOOL={tool} to assign a lane to tool"
            " {tool}, then use TR_RESUME to continue.".format(tool=str(tool))
        )
        ...
        resume_kwargs = {
            ...
            "action": lambda g=gcmd: self.cmd_TR_LOAD_TOOLHEAD(g),
            "fail_msg": (
                "Cannot resume. Please use TR_ASSIGN_LANE to assign a lane to"
                " tool %d, then use TR_RESUME." % tool
            ),
        }
        self._set_up_resume("check condition", resume_kwargs)
