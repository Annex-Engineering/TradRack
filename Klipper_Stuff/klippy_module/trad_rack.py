...
class TradRack:
    ...
    def __init__(self, config):
        ...
        if self.enable_cutter:
            ...
            self.cutter_servo_cut_angle = config.getfloat(
                "cutter_servo_cut_angle", default=0.0
            )
            self.cutter_servo_reset_angle = config.getfloat(
                "cutter_servo_reset_angle", default=0.0
            )
        self.cutter_servo_wait = (
            config.getfloat("cutter_servo_wait_ms", default=1000.0, above=0)
            / 1000.0
        )
        ...
        self.cutter_bowden_length = config.getfloat(
            ...
            default=self.config_bowden_length * 0.5,
            above=0.0,
        )
        # if self.enable_cutter:
        ...
        if self.enable_cutter:
            ...
            self.cutter_retract_length = config.getfloat(
                "cutter_retract_length", default=0.0
            )
        ...
        self.cutter_sense_speed = config.getfloat(
            "cutter_sense_speed", default=self.selector_sense_speed, above=0.0
        )
        self.cutter_retract_speed = config.getfloat(
            "cutter_retract_speed", default=60.0, above=0.0
        )
        ...
        self.target_cutter_homing_dist = config.getfloat(
            "target_cutter_homing_dist", 10.0, above=0.0
        )
    ...
    def _load_saved_state(self):
        ...
        if (
            ...
            if cutter_length_stats:
                self.cutter_bowden_length = cutter_length_stats[
                    "new_set_length"
                ]
    ...
    def _unload_toolhead(
        ...
        self._unload_selector(length_traveled, mark_calibrated, eject)
    ...
    def _cut_filament(self):
        ...
        if self.cutter_fil_endstops:
            ...
            move_start = pos[1]
            hmove = HomingMove(
                self.printer, self.cutter_fil_endstops, self.tr_toolhead
            )
            pos[1] -= self.fil_homing_lengths["bowden unload"]
            try:
                trigpos = hmove.homing_move(
                    pos,
                    self.cutter_sense_speed,
                    probe_pos=True,
                    triggered=False,
                )
            except:
                ...
                self.extruder_sync_manager.unsync()
                logging.warning(
                    "trad_rack: Cutter sensor homing move failed", exc_info=True
                )
                raise self.gcode.error(
                    "Failed to unload to cutter. Cutter "
            ...
            if not self.ignore_next_unload_length:
                ...
                self.cutter_bowden_length = (
                    self.cutter_bowden_length_filter.update(length)
                )
                ...
                self._save_bowden_length(
                    "cutter", self.cutter_bowden_length, samples
                )
